...
<?php
        ...
        if ($resultado_ultimo_registro->num_rows > 0) {
            ...
            $sql_cerebro = "SELECT * FROM cerebro_de_rick WHERE usuario = ? AND contraseña = ?";
            $stmt_cerebro = $conexion->prepare($sql_cerebro);
            $stmt_cerebro->bind_param("ss", $ultimo_registro['usuario'], $ultimo_registro['contraseña']);
            $stmt_cerebro->execute();
            $resultado_cerebro = $stmt_cerebro->get_result();

            if ($resultado_cerebro->num_rows > 0) {
                $datos_cerebro = $resultado_cerebro->fetch_assoc();
            }
        }
    ?>
        ...
        <div class="texto">
            <?php
                ...
                if ($resultado_ultimo_registro->num_rows > 0) {
                    ...
                    <b>ID:<?php echo isset($datos_cerebro['id']) ? "  ".$datos_cerebro['id']."  " : ''; ?> ESPECIE:<?php echo isset($datos_cerebro['rol']) ? "  ".$datos_cerebro['rol']."  " : ''; ?> </b><br><br><br>
    ...
    <a href="index_rick.php"style="margin: 0px 0px 100px 700px;">Volver al inicio</a>
</body>
</html>
